...
public abstract class BlocksterObject extends ScaledObject {
    ...
    public Block getAdjacentBlock() {
        Block block = new EmptyBlock((int) (getX() / getScaleX()), (int) (getY() / getScaleY()), blockMap);

        if (dir == Direction.LEFT) {
            block = blockMap.getBlock((int) (getX() / scaleX) - 1, (int) ((2 * getY() + getHeight()) / 2 / scaleY));

        }

        if (dir == Direction.RIGHT) {
            block = blockMap.getBlock((int) ((getX() + getWidth()) / scaleX) + 1,
    ...
    public void moveToNextPosition() {
        ...
        LOG.log(Level.INFO, "Moved " + this + " (" + direction + ") " + anim.getMovement() + ") (" + anim + ")");
    }
    ...
    public String toString() {
        return this.getClass().getSimpleName() + ": (" + Math.round(getX() * 10) / (10 * scaleX) + ", "
