...
public class GdxView implements ApplicationListener, Disposable {
    ...
    public void transitCamera() {
        ...
        background.setPosition(camera.position.x * 0.7f - background.getWidth() * 2, camera.position.y * 0.7f
                - background.getHeight() * 2);

        final boolean cameraInPlace = camera.position.epsilonEquals(model.getActivePlayer().getX(), model
                .getActivePlayer().getY(), 0, 30f);
    ...
    public PlayerView createPlayerView(Player player) {
        ...
        return new PlayerView(player, animF.getArrayOfAnimations(), animF.getWalkAnimations(), texr);
    }
}
